...
function usage(): string {
  return 'Usage: pnpm study:export -- --output <directory> [--database <study.sqlite>]\n';
}
...
function parseExportOptions(argumentsList: readonly string[]): ExportCommandOptions | null {
  ...
  for (let index = 0; index < argumentsList.length; index += 1) {
    ...
    if ((argument === '--database' || argument === '--output') && value === undefined) return null;
  ...
  return outputDirectory === null ? null : { databasePath, outputDirectory };
}
...
if (options === null) {
  ...
  try {
    ...
    process.stdout.write(
      `Research export created with ${result.files.length} files and ${result.manifest.files.length} data hashes.\n`,
    );
  } catch (error) {
