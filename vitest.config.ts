import { defineConfig } from 'vitest/config';

export default defineConfig({
  test: {
    environment: 'node',
    include: [
      'packages/password-analysis/src/password-analysis.test.ts',
      'packages/password-analysis/src/password-candidate-corpus.test.ts',
      'packages/contracts/src/contracts.test.ts',
      'packages/study-engine/src/study-machine.test.ts',
      'packages/study-engine/src/timing.test.ts',
      'packages/training-content/src/s00-s01.traceability.test.ts',
      'packages/training-content/src/s03.traceability.test.ts',
      'packages/training-content/src/s04.traceability.test.ts',
      'packages/training-content/src/s05.traceability.test.ts',
      'packages/training-content/src/s06-s07.traceability.test.ts',
      'packages/training-content/src/s12.traceability.test.ts',
      'packages/training-content/src/s13.traceability.test.ts',
      'packages/training-content/src/s14.traceability.test.ts',
      'packages/training-content/src/s15-s17.traceability.test.ts',
      'apps/study-web/src/api/study-api.test.ts',
      'apps/study-web/src/features/training/segments/S05/S05ComponentStrategy.test.ts',
      'apps/study-web/src/features/training/segments/S06/S06ConsequenceController.test.ts',
      'apps/study-web/src/features/training/supportive-reload-checkpoint.test.ts',
      'apps/study-web/src/features/training/segments/S07/S07PassphraseSearchMachine.test.ts',
      'apps/study-web/src/features/training/segments/S08/S08ProtectionModel.test.ts',
      'packages/visualization/src/account-exploration-scene.test.ts',
      'packages/visualization/src/password-consequence-scene.test.ts',
      'packages/training-engine/src/password-module-machine.test.ts',
      'packages/training-engine/src/password-module-controller.test.ts',
      'packages/training-engine/src/password-module-privacy.test.ts',
      'apps/study-server/src/app.test.ts',
      'apps/study-server/src/followup-operations.test.ts',
      'apps/study-server/src/recontact.test.ts',
      'apps/study-server/src/research-export.test.ts',
      'apps/study-server/src/runtime-restart.test.ts',
      'apps/study-server/src/study-data-deletion.test.ts',
      'apps/study-server/src/web-release-acceptance.test.ts',
    ],
    passWithNoTests: false,
    restoreMocks: true,
    testTimeout: 30_000,
  },
});
